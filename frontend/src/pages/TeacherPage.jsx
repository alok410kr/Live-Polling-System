...
const TeacherPage = () => {
  ...
  // Join as teacher when component mounts and request state recovery
  useEffect(() => {
    socket.emit("joinAsTeacher");
    // Request current poll state for state recovery
    socket.emit("getCurrentPollState");
    return () => {
      // Cleanup if needed
    };
  }, []);
  ...
  const handleAskQuestion = () => {
    ...
    if (uniqueOptions.length !== validOptions.length) {
      alert(
        "Duplicate options are not allowed! Each option must be unique. Please make sure all options have different text."
      );
      return;
    }
    ...
    const correctOptions = options.filter(
      (opt) => opt.isCorrect === true && opt.text.trim()
    );
            ...
            >
              {currentPoll.options &&
                currentPoll.options.map((option, index) => {
                  // Show results if poll is active (with real-time updates) or ended
                  // Always show results for active polls, even if no votes yet
                  const showResults =
                    currentPoll.status === "active" ||
                    currentPoll.status === "ended";
                  const votes =
                    showResults && results ? results.results?.[option] || 0 : 0;
                  const total =
                    showResults && results ? results.totalVotes || 0 : 0;
                  // Show 0% if no votes, otherwise calculate percentage
                  const percentage = showResults ? (total > 0 ? (votes / total) * 100 : 0) : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Percentage - Show when poll is active or ended */}
                      {showResults && (
                        <span
                          style={{
                            fontFamily: "Sora, sans-serif",
                            ...
                            fontWeight: 600,
                            color:
                              isCorrect && currentPoll.status === "ended"
                                ? "#10B981"
                                : "#7565D9",
                            minWidth: "50px",
                            textAlign: "right",
                        ...
                        >
                          {total > 0 ? `${percentage.toFixed(0)}%` : "0%"}
                        </span>
                      )}

                      {/* Progress Bar - Show when poll is active or ended */}
                      {showResults && (
                        <div
                          style={{
                            width: "200px",
            ...
            >
              ...
              <div ref={timerDropdownRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  >
                    <polygon points="9.5,0 19,18 0,18" fill="#480FB3" />
                  </svg>
                </button>
                ...
                >
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        handleOptionChange(option.id, "isCorrect", false)
                      }
                      style={{ cursor: "pointer" }}
                    />
                    <span
